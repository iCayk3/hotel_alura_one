...
import br.com.hotel_alura.controller.ControllerHospede;
import br.com.hotel_alura.controller.ControllerReserva;
...
public class Buscar extends JFrame {
	...
	int xMouse, yMouse;
	private ControllerHospede ctrlHospede = new ControllerHospede();
	private ControllerReserva ctrlReserva = new ControllerReserva();
	...
	public Buscar() {
		...
		btnbuscar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(modeloHospedes.getRowCount() != 0){
					limparTela();
				}	
		...
		btnEditar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					if(tbHospedes.getSelectedRowCount() > 0){
						alterarHospede();
					}
	 ...
	 private void headerMousePressed(java.awt.event.MouseEvent evt) {
	    ...
	    private void alterarHospede(){
	    	...
	    	if (objetoDaLinha instanceof Integer) {
				...
				this.ctrlHospede.alterar(nome, sobrenome, dataNascimento, nacionalidade, telefone, id);
	    ...
	    private void alterarReserva(){
	    	...
	    	if (objetoDaLinha instanceof Integer) {
					...
					Integer id = (Integer) objetoDaLinha;
					String dataE = (String) modelo.getValueAt(tbReservas.getSelectedRow(), 1);
					String dataS = (String) modelo.getValueAt(tbReservas.getSelectedRow(), 2);
					try{
						valor = Float.valueOf((String)modelo.getValueAt(tbReservas.getSelectedRow(), 3));
					}catch(java.lang.ClassCastException erroL){
					...
					this.ctrlReserva.alterar(dataE, dataS, valor, formaPagamento, id);
					JOptionPane.showMessageDialog(this, "Informações atualizadas com sucesso!");
	    ...
	    private void listaTodosHospedes(){
	    	List<Hospede> hospedes = ctrlHospede.listarTodos();
			
			for(Hospede hospede:hospedes){
				modelo.addRow(new Object[]{
					hospede.getReserva().getId(), hospede.getReserva().getDataEntrada(), hospede.getReserva().getDataSaida(),
					hospede.getReserva().getValor(), hospede.getReserva().getFormaPagamento()
				});
				modeloHospedes.addRow(new Object[]{	
					hospede.getId(), hospede.getNome(), hospede.getSobrenome(), hospede.getDataNascimento(), hospede.getNacionalidade(), hospede.getTelefone(),
					hospede.getReserva().getId()
				});
			}	
	    }
	    
	    private void listaHospedeFiltrados(){
	    	List<Hospede> hospedes = ctrlHospede.recuperaComFiltros(txtBuscar.getText());
			for(Hospede hospede:hospedes){
				modeloHospedes.addRow(new Object[]{	
					hospede.getId(), hospede.getNome(), hospede.getSobrenome(), hospede.getDataNascimento(), hospede.getNacionalidade(), hospede.getTelefone(),
					hospede.getReserva().getId()
	    ...
	    private void deletarHospede() {
			...
			if (objetoDaLinha instanceof Integer) {
				Integer id = (Integer) objetoDaLinha;
				Hospede hospede = ctrlHospede.buscarPorId(id);
				System.out.println(hospede);
				this.ctrlHospede.deletar(id);
				ctrlReserva.deletar(hospede.getReserva().getId());
